...
export class MonitoreoTemperaturaHistoryService {
  ...
  /**
   * Obtiene informes filtrados por planta y fecha desde la API
   * @param planta Código de la planta (opcional)
   * @param fecha Fecha en formato YYYY-MM-DD (opcional)
   * @returns Observable con la respuesta de informes
   */
  getInformesPorPlantaFecha(planta?: string, fecha?: string): Observable<InformesResponse> {
    let params = new HttpParams();
    
    if (planta) {
      params = params.append('planta', planta);
    }
    ...
    return this.http.get<InformesResponse>(`${this.apiUrl}/planta-fecha`, { params });
  }

  /**
   * Obtiene un informe específico por su ID
   * @param id ID del informe
   * @returns Observable con el informe solicitado
   */
  getInformePorId(id: number): Observable<InformeItem> {
    return this.http.get<InformeItem>(`${this.apiUrl}/${id}`);
  }
   ...
   * @returns Observable con la respuesta del servidor
   */
  actualizarInforme(id: number, informe: Partial<InformeItem>): Observable<InformeItem> {
    return this.http.put<InformeItem>(`${this.apiUrl}/${id}`, informe);
  }
